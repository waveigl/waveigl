...
export async function POST(request: NextRequest) {
  try {
    ...
    const { data: linked, error } = await db
      ...
      .select('platform_username, platform_user_id, access_token, refresh_token, expires_at')
    ...
    switch (platform) {
      ...
      case 'kick':
        ...
        const expiresAt = linked.expires_at ? new Date(linked.expires_at).getTime() : 0
        const isExpired = expiresAt > 0 && Date.now() > (expiresAt - 60000) // 1 minuto de margem

        // Se expirado ou prestes a expirar, renovar antes de enviar
        if ((!kickToken || isExpired) && linked.refresh_token) {
          console.log('[Kick] Token expirado ou próximo da expiração, renovando proativamente...')
        ...
        // Enviar mensagem
        let kickResult = await sendKickMessage(linked.platform_username, kickToken, message, linked.platform_user_id)

        // Se ainda assim falhou (ex: o token foi invalidado remotamente), tentar UMA vez a renovação reativa
        if (!kickResult && linked.refresh_token) {
          console.log('[Kick] Envio falhou, tentando renovação reativa...')
          const newToken = await refreshKickToken(linked.refresh_token, session.userId, db)

          if (newToken) {
            kickResult = await sendKickMessage(linked.platform_username, newToken, message, linked.platform_user_id)
          }
        }
